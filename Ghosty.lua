...
function nukeserver()
    Citizen.CreateThread(function()
        ...
        for bs=0,22 do 
            TriggerServerEvent("_chat:messageEntered"," ^8Ghosty 1.1",{141,211,255},"")
        end
...
function tabs() 
	if tab == "Self" then
		...
		ScreenText("~w~[~r~Ghosty Menu~w~]  ~y~", 4, 1.0, 0.5, 0.97, 0.35, 255, 255, 255, 225)
		...
		if SlimButton(SuiName,false,0.4,0.465) then
			...
			AddNotification("~p~SYSTEM ~s~Slå safemode fra!!")
	end
end
		
			if otc_xd.locals.SafeMode == false then
				SkiName = "~p~ Select Skin    ~w~↓"
			else
...
--	end
	...
	elseif tab == "Online" then
		...
		if SlimButton(""..pref.."Penge 100k (Devo)",false,0.54,0.535) then
			if otc_xd.locals.SafeMode == false then
			Citizen['CreateThread'](function()
				for k, v in pairs(GetActivePlayers()) do
					Citizen['Wait'](50)
					TriggerServerEvent('trucker:success',100000)
					AddNotification("Spawnede 100.000!")
				end
			end)
		else
			AddNotification("~p~SYSTEM ~s~Slå safemode fra!!")
		...
		if SlimButton(""..pref.."Penge 10 k (Devo)",false,0.60,0.465) then
			if otc_xd.locals.SafeMode == false then
			TriggerServerEvent('trucker:success',10000)
			AddNotification("Spawnede 10.000!")
		else
			AddNotification("~p~SYSTEM ~s~Slå safemode fra!!")
				end
		end
	...
	elseif tab == "Troll" then
		...
		--	end
			...
			if SlimButton(""..pref.."Send Bills",false,0.4,0.36) then --535
				if otc_xd.locals.SafeMode == false then
                for i = 0, 128 do
                    TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Ghosty Menu", "99999999")
                end
			else
				AddNotification("~p~SYSTEM ~s~Slå safemode fra!!")
					end
	...
	elseif tab == "Visual" then
		...
		if SlimButton("ESP M. Color",false,0.4,0.36) then
			...
			local g = KeyBoardInput("~p~Green", "", 3)
	...
	elseif tab == "Vehicle" then
		...
		if SlimButton("Spawn Veh w/c",false,0.4,0.465) then
			...
			local r, g, b = KeyBoardInput("~r~Red", "", 3), KeyBoardInput("~p~Green", "", 3), KeyBoardInput("~b~Blue", "", 3)
...
function meni()
	----------- MAIN ----------
	text("~p~Ghosty 1.1",false,0.47,0,0.355, 0.255, true, 6)
